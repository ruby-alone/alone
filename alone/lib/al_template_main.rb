...
class AlTemplate
  ...
  EMBEDDED_PATTERN = /<%(=|-|\#|%)?(.*?)([-=])?%>([ \t]*\r?\n)?/m
  COMMAND_EXTRACTOR = /\A *(include|expand|header_section|body_section|footer_section|h|u|s)(([ (].*)|\z)/
  IS_ALL_SPACE = /\A[ \t]*\z/
  IS_QUOTED_TEXT = /\A[\(\s]*["']([^"']+)["'][\)\s]*\z/
  ...
  ##
  # プリプロセスコマンド　静的コンテンツサポート
  #
  def cmd_s( src, param )
    if IS_QUOTED_TEXT =~ param
      add_text( src, Alone::escape_html("#{AL_URI_STATIC}/#{$1}") )
    else
      add_expr_literal( src, "Alone::escape_html('#{AL_URI_STATIC}/' + #{param})" )
    end
  end
